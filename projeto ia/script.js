...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const comorbiditiesChart = new Chart(comorbiditiesCtx, {
        type: 'pie',
        data: {
    ...
    setInterval(() => {
        ...
        // Atualiza dados do gráfico de glicemia
        glucoseChart.data.datasets[0].data = glucoseChart.data.datasets[0].data.map(
            value => value + (Math.random() * 8 - 4)
        );
        glucoseChart.data.datasets[1].data = glucoseChart.data.datasets[1].data.map(
            value => value + (Math.random() * 12 - 6)
        );
        glucoseChart.update();
        ...
        // Atualiza dados do gráfico de comorbidades
        comorbiditiesChart.data.datasets[0].data = comorbiditiesChart.data.datasets[0].data.map(
            value => value + (Math.random() * 3 - 1.5)
        );
        comorbiditiesChart.update();
    }, 5000);
